...
const generateTransactionId = () => {
  ...
  const length = 10;
  return 'TXN' + Array.from({ length }, () => 
    chars.charAt(Math.floor(Math.random() * chars.length))
  ).join('');
};

function Cart() {
  const { cartItems, instructions, updateQuantity, clearCart, updateInstructions } = useCart();
      ...
      >
        <div id="receipt-content" className="bg-white p-4 sm:p-6">
          <div className="text-center border-b pb-3 mb-4">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Order Receipt</h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="font-semibold text-sm">{orderDetails?.transactionId}</p>
            </div>
            ...
            <div>
              ...
              <p className="font-semibold text-sm">{orderDetails?.phoneNumber}</p>
            </div>

            <div>
              <p className="text-gray-600 text-xs sm:text-sm mb-2">Order Items:</p>
                ...
                {orderDetails?.items.map((item) => (
                  <div key={item.id} className="flex justify-between items-start border-b pb-2">
                    <div className="pr-2">
                      ...
                      {item.instructions && (
                        <p className="text-xs text-gray-500 mt-0.5">Note: {item.instructions}</p>
                      )}
                    </div>
            ...
            <div className="border-t pt-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-sm sm:text-base">₹{orderDetails?.totalAmount}</span>
              </div>
            </div>

            <div className="border-t pt-3">
              <p className="text-xs sm:text-sm text-gray-500 text-center">
                Order Time: {new Date(orderDetails?.orderTime).toLocaleString('en-US', {
                  dateStyle: 'medium',
                  timeStyle: 'short',
                  hour12: true
                })}
